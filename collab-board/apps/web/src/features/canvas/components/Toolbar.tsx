"use client";
import{Pencil,Minus,Square,Circle,Eraser,Undo2,Trash2,Download,ChevronRight,ChevronDown}from "lucide-react";
...
export default function Toolbar({tool,color,brushSize,canWrite,isAdmin,onToolChange,onColorChange,onBrushSizeChange,onUndo,onClear,onExport,onExpand}:ToolbarProps){
  if(!canWrite)return <div className="absolute left-3 top-1/2 -translate-y-1/2 z-20 flex flex-col items-center gap-1 p-1 bg-gray-900/95 border border-gray-700 rounded-xl shadow-xl backdrop-blur-sm max-h-[85vh] overflow-y-auto" style={{scrollbarWidth:"none"}}><span className="text-[10px] text-gray-500 px-1 py-0.5">View Only</span><Divider/><ToolBtn onClick={onExport} title="Export PNG"><Download className="w-4 h-4"/></ToolBtn></div>;
  return(
    <div className="absolute left-3 top-1/2 -translate-y-1/2 z-20 flex flex-col items-center gap-1 p-1 bg-gray-900/95 border border-gray-700 rounded-xl shadow-xl backdrop-blur-sm select-none max-h-[85vh] overflow-y-auto" style={{scrollbarWidth:"none"}}>
